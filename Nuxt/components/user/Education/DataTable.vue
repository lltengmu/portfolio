...
const { editing, load, data, remove, form } = useEducations()
await load()
...
const columns: ColumnDef<Education>[] = [
  ...
  },
  {
    ...
    header: 'Institution',
    cell: ({ row }) => h('div', { class: 'capitalize' }, row.getValue('institution')),
  },
  {
    accessorKey: 'degree',
    header: 'Degree',
    cell: ({ row }) => h('div', { class: 'capitalize' }, row.getValue('degree')),
  },
  {
        ...
        },
        {
          ...
          onClick: () => {
            Object.assign(form, {
              ...
              company: row.original.institution,
              position: row.original.degree,
            })
            editing.value = true
          },
        },
